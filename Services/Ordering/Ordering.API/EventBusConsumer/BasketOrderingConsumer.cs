...
public class BasketOrderingConsumer : IConsumer<BasketCheckoutEvent>
{
    ...
    {
        using var scope = _logger.BeginScope($"Consuming Basket Checkout Event for {context.Message.CorrelationId}, for {context.Message.UserName} user");

        _logger.LogInformation($"---> Consuming Basket Checkout Event for {context.Message.CorrelationId}, for {context.Message.UserName} user");
